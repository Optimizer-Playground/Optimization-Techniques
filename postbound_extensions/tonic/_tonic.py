...
def _normalize_filter_predicate(
    ...
    renamed_columns = {
        col: pb.ColumnReference(col.name, renamed_tables[col.table])
    ...
    def recommend_operators(
        ...
        *,
        ...
        if next_join.is_bushy():
            ...
            qeps_subquery_node = self.child_nodes.get(qeps_subquery_id)
            if qeps_subquery_node is None:
                return
        ...
        qeps_child_node = self.child_nodes.get(qeps_child_id)
        if qeps_child_node is None:
            return
...
def _load_qeps_from_json(
    ...
    subquery = (
        ...
        if "subquery" in json_data and json_data["subquery"] is not None
        else None
    )
    ...
    for child_json in json_data.get("children", []):
        ...
        child_node = _load_qeps_from_json(child_json["node"], child_id, node, filter_aware, gamma)
