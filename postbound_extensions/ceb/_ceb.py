...
class PredicateGenerator:
    ...
    db_connection : Optional[pb.Database], optional
    ...
    def choose_predicate_values(self) -> None:
        ...
        if self._max_tries is None:
            assert self._parent_generator is not None
            self._max_tries = self._parent_generator.max_tries

        selected_value: tuple[PlaceHolderValue] | None = None
        current_try = 0
...
class QueryTemplate:
    ...
    def generate_query(self) -> pb.SqlQuery:
        """Creates a new SQL query by replacing all placeholders in the base query with appropriate values."""
        final_query = self.generate_raw_query()
        return pb.parse_query(final_query)
    ...
    def _lookup_column(self, colname: ColumnName) -> pb.ColumnReference:
        ...
        table_ref = pb.TableReference(target_table, table)
        return pb.ColumnReference(column, table_ref)
...
def _parse_template_toml(path: str | Path, db_connection: pb.Database) -> QueryTemplate:
...
def generate_raw_workload(
    ...
    *,
    ...
    generate_workload
    """
    db_connection = db_connection or pb.db.current_database()
    template_dir = path if isinstance(path, Path) else Path(path)
...
def persist_workload(
    ...
    for label, query in workload.entries():
        ...
        with open(query_file, "w") as query_file:
            query_file.write(pb.qal.format_quick(query) + "\n")
