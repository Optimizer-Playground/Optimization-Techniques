# < begin copyright >
# Copyright Ryan Marcus 2019, Rico Bergmann 2026
#
...
type PreorderIndexes = int | tuple[int, PreorderIndexes, PreorderIndexes]
...
def _flatten[NodeT](
    root: NodeT,
    transformer: NodeFlatten[NodeT],
    left_child: TreeTraversal[NodeT],
    right_child: TreeTraversal[NodeT],
) -> np.ndarray:
...
def _pad_and_combine(xs: list[np.ndarray]) -> np.ndarray:
    assert len(xs) >= 1
    ...
    max_first_dim = max(arr.shape[0] for arr in xs)
